...
export default function ConnectSocialPage() {
  ...
  // Which verification method the creator picked (null = still choosing).
  ...
  const [manualVerifications, setManualVerifications] = useState<any[]>([]);
  ...
  const automationStat = automationStatus(connectedAccounts, oauthError);
  const manualStat = manualStatus(manualVerifications);
    ...
    async function loadData() {
      ...
      try {
        ...
        if (tokenRes.ok) {
          ...
          setManualVerifications(tokenData.verifications ?? []);
        }
      } catch {
        // tokens are optional — the form still works without them
      }

      // The OAuth callback bounces back here with ?success=instagram or
      // ?error=oauth_failed — reflect that on the automation box.
      const params = new URLSearchParams(window.location.search);
      if (params.get("error")) setOauthError(true);
      if (params.get("success")) setOauthSuccess(params.get("success"));
  ...
  async function persistTierAndAdvance(counts: number[]) {
    if (!userId) return;
    const supabase = createSupabaseBrowserClient();
    const max = Math.max(0, ...counts);
    const computedTier = getCreatorTier(max);
    await supabase
      .from("creator_profiles")
      .update({
        onboarding_step: "plan_selection",
        ...(computedTier
          ? { tier: computedTier, previous_tier: computedTier, tier_changed_at: new Date().toISOString() }
          : {}),
      })
      .eq("user_id", userId);
  ...
  async function submitVerification() {
    ...
    await persistTierAndAdvance([followerCount]);
  }
      ...
      <CardContent>
        {oauthSuccess && (
          <div className="mb-4 rounded-lg border border-success/30 bg-success/10 p-3 text-sm text-success">
            ✓ {oauthSuccess === "instagram" ? "Instagram" : oauthSuccess === "tiktok" ? "TikTok" : oauthSuccess} connected and verified. Your follower count is now live.
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            {connectedAccounts.length > 0 && (
              <div className="rounded-lg border border-border bg-muted/50 p-3">
                ...
                {connectedAccounts
                  .filter((acc) => !acc.disconnected_at)
                  .map((acc) => (
        ...
        {method === "automation" && (
          <div className="space-y-4">
            <AutomationSetup
              ...
              onConnected={() => {
                // Re-fetch connected accounts so the "Continue" gate updates.
                const supabase = createSupabaseBrowserClient();
                supabase
                  .from("creator_social_accounts")
                  .select("*")
            ...
            <div className="flex justify-between">
              <Button variant="outline" onClick={() => setMethod(null)}>
                <ChevronLeft className="h-4 w-4" /> Back to methods
              </Button>
              <div className="flex gap-2">
                ...
                <Button
                  onClick={() =>
                    persistTierAndAdvance(
                      connectedAccounts
                        .filter((a) => !a.disconnected_at)
                        .map((a) => Number(a.follower_count ?? 0)),
                    )
                  }
                  disabled={automationStat !== "completed"}
                >
                  Continue <ArrowRight className="h-4 w-4" />
                </Button>
              </div>
            </div>
            {automationStat !== "completed" && (
              <p className="text-xs text-muted-foreground">
                Connect at least one platform to continue. Twitter/X has no automation API — use
                manual sign up for it.
              </p>
            )}
          </div>
        )}
